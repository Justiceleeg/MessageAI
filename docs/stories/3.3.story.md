...
- [ ] Task 1: Add Firebase Realtime Database SDK (AC: 6)
  - [ ] Add Firebase Realtime Database package dependency to Xcode project
  - [ ] Import FirebaseDatabase in necessary files
  - [ ] Initialize Realtime Database in app startup (Firebase.configure() already done)
  - [ ] Verify Realtime Database is enabled in Firebase Console
  - [ ] Document Realtime Database setup for team reference
  - [ ] Note: Presence data will use RTDB, all other data stays in Firestore

- [ ] Task 2: Create PresenceService with Realtime Database (AC: 1, 2, 5, 6)
  - [ ] Create PresenceService.swift in Services directory
  - [ ] Use Firebase Realtime Database reference: Database.database().reference()
  - [ ] Implement goOnline(userId: String) method
  - [ ] Write to RTDB path: users/{userId}/presence with { online: true, lastSeen: timestamp }
  - [ ] Implement goOffline(userId: String) method  
  - [ ] Write to RTDB path: users/{userId}/presence with { online: false, lastSeen: timestamp }
  - [ ] Use onDisconnect() to automatically set offline when connection drops
  - [ ] Set onDisconnect handler when going online: ref.onDisconnectSetValue({ online: false, lastSeen: timestamp })
  - [ ] NO HEARTBEAT TIMER NEEDED - onDisconnect handles it automatically!
  - [ ] Add 3-second debouncing for rapid foreground/background transitions
  - [ ] Add comprehensive debug logging for all presence state changes
  - [ ] Add error handling for RTDB write failures
  - [ ] MANUAL TESTING: Test presence updates with app switching

- [ ] Task 3: Create Presence Listening Helper Methods (AC: 1, 2, 6)
  - [ ] Add helper methods to PresenceService for listening to presence
  - [ ] Method: observePresence(userId: String, completion: (Bool, Date) -> Void) -> DatabaseHandle
  - [ ] Listen to RTDB path: Database.database().reference().child("users/{userId}/presence")
  - [ ] Listen to RTDB value changes with .observe(.value) { snapshot in ... }
  - [ ] Parse snapshot: { "online": Bool, "lastSeen": timestamp }
  - [ ] Return DatabaseHandle for cleanup
  - [ ] Method: stopObservingPresence(userId: String, handle: DatabaseHandle)
  - [ ] Calls: ref.child("users/\(userId)/presence").removeObserver(withHandle: handle)
  - [ ] Add error handling for RTDB listener failures
  - [ ] Add debug logging for listener start/stop and presence changes
  - [ ] NOTE: Actual usage will be in Task 4 (conversation list) and Task 5 (chat view)
  - [ ] MANUAL TESTING: Test listener setup and cleanup with debug logs

- [ ] Task 4: Display Presence Indicators in Conversation List (AC: 1, 2)
  - [ ] Update ConversationListView to show presence indicators
  - [ ] **OPTIMIZATION: Only track presence for conversations visible on screen**
  - [ ] Use SwiftUI .onAppear() and .onDisappear() on each conversation row
  - [ ] When conversation row appears: Start RTDB listener for that user's presence
  - [ ] When conversation row disappears: Stop RTDB listener for that user
  - [ ] Store active listeners in ConversationListViewModel: var activePresenceListeners: [String: DatabaseHandle]
  - [ ] For 1:1 chats: Show green dot if other user is online, gray if offline
  - [ ] For group chats: Skip presence indicators in conversation list (too complex)
  - [ ] Position indicator as overlay on bottom-right of avatar circle
  - [ ] Use SF Symbol: circle.fill (green for online, gray if offline)
  - [ ] Size: 12pt diameter with 2pt white border
  - [ ] Add smooth animation when presence changes (fade transition)
  - [ ] Add accessibility labels: "User is online" / "User is offline"
  - [ ] NOTE: Infinite scroll will work naturally with this approach - only visible rows tracked
...
- [ ] Task 5: Display Presence in Chat View (AC: 1, 2, 3, 4)
  - [ ] **For 1:1 Chats:**
  - [ ] Add presence subtitle in navigation bar: "Online" or "Last seen X ago"
  - [ ] Add presence listener in ChatViewModel for the other user (RTDB)
  - [ ] Update subtitle in real-time when presence changes
  - [ ] Format last seen: "Online", "Last seen 5m ago", "Last seen 2h ago", "Last seen yesterday"
  - [ ] Green text for "Online", gray text for "Last seen..."
  - [ ] **For Group Chats:**
  - [ ] Add green dot indicator to the right of sender name label for online users
  - [ ] Position: Small green circle.fill SF Symbol after sender name in MessageBubbleView
  - [ ] Only show green dot if sender is currently online (query presence from RTDB)
  - [ ] Add presence listeners for all group participants when ChatView opens
  - [ ] Store presence in ChatViewModel: @Published var participantPresence: [String: Bool]
  - [ ] Update MessageBubbleView to accept isOnline parameter
  - [ ] Add accessibility: "User is online" for green dot
  - [ ] **Both:**
  - [ ] Add helper method: formatLastSeen(_ date: Date) -> String
  - [ ] Handle listener cleanup when ChatView disappears
  - [ ] MANUAL TESTING: Test presence indicators in 1:1 and group chats

- [ ] Task 6: Implement App Lifecycle Hooks (AC: 5, 6)
  - [ ] Hook into SwiftUI app lifecycle using @Environment(\.scenePhase)
  - [ ] Alternative: Use AppDelegate/SceneDelegate lifecycle methods
  - [ ] Call PresenceService.goOnline(userId) when app becomes active (.active)
  - [ ] Call PresenceService.goOffline(userId) when app enters background (.background, .inactive)
  - [ ] Handle applicationWillTerminate: Call goOffline() (best effort)
  - [ ] Implement 3-second debouncing for rapid transitions (learning from 3.2)
  - [ ] Add debug logging for all lifecycle events
  - [ ] NOTE: onDisconnect() auto-handles crashes/force-quits
  - [ ] MANUAL TESTING: Test app switching, backgrounding, termination, force-quit

- [ ] Task 7: Realtime Database Security Rules for Presence (AC: 5, 6)
  - [ ] Create/update RTDB Security Rules for presence data (different syntax than Firestore!)
  - [ ] Allow any authenticated user to READ presence: .read: "auth != null"
  - [ ] Allow only the user to WRITE their own presence: .write: "auth.uid == $userId"
  - [ ] RTDB Rules JSON structure:
    ```
    {
      "rules": {
        "users": {
    ...
    ```
  - [ ] Test security rules in Firebase Console → Realtime Database → Rules tab
  - [ ] Use Rules Simulator to verify read/write permissions
  - [ ] Document RTDB rules in architecture docs
  - [ ] MANUAL TESTING: Test unauthorized access attempts

- [ ] Task 8: Handle Edge Cases and Error Scenarios (AC: 2, 5, 6)
  - [ ] Handle network errors when updating presence (RTDB handles reconnection automatically)
  - [ ] NOTE: onDisconnect() automatically handles connection drops - no manual retry needed!
  - [ ] Handle app termination: Best-effort goOffline() call
  - [ ] Handle rapid app switching (foreground/background/foreground) with 3-second debouncing
  - [ ] Handle case where RTDB listener fails: Add error callback, log, and attempt reconnection
  - [ ] Use RTDB ServerValue.TIMESTAMP for consistency (not client timestamps)
  - [ ] Add comprehensive error logging with context (userId, action, error message)
