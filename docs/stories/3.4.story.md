...
- [ ] Task 1: Create NotificationManager Service (AC: 1, 6)
  - [ ] Create NotificationManager.swift in Services directory
  - [ ] Define NotificationManager as @Observable class
  - [ ] Add property to track currently viewed conversationId (for suppression)
  - [ ] Add method: requestNotificationPermissions() → Bool
  - [ ] Add method: scheduleLocalNotification(conversationId, senderName, messageText)
  - [ ] Add method: shouldSuppressNotification(conversationId) → Bool
  - [ ] Add method: showInAppBanner(conversationId, senderName, messageText)
  - [ ] Add published property for in-app banner state
  - [ ] Add comprehensive debug logging for all operations
  - [ ] Add unit tests for NotificationManager logic

- [ ] Task 2: Request Local Notification Permissions (AC: 3)
  - [ ] Request notification permissions on app launch (after authentication)
  - [ ] Use UNUserNotificationCenter.requestAuthorization(options: [.alert, .sound, .badge])
  - [ ] Store permission status in UserDefaults
  - [ ] Handle permission grant: Set up notification center delegate
  - [ ] Handle permission denial: Show informational message (don't block app)
  - [ ] Don't repeatedly prompt if user has denied
  - [ ] Add debug logging for permission requests and responses
  - [ ] Add unit tests for permission handling logic

- [ ] Task 3: Implement Firestore Message Listener for Notifications (AC: 1, 5)
  - [ ] Create new listener in NotificationManager or ChatViewModel
  - [ ] Listen to all conversations where user is a participant
  - [ ] Detect new messages created in Firestore (use snapshot metadata)
  - [ ] Filter: Only trigger for messages from other users (not self)
  - [ ] Check: Is user currently viewing this conversation? (suppression logic)
  - [ ] If not viewing: Trigger notification (in-app or local based on app state)
  - [ ] Extract message data: conversationId, senderName, messageText
  - [ ] Handle listener errors gracefully
  - [ ] Add comprehensive debug logging
  - [ ] Ensure listener cleanup on logout/app termination

- [ ] Task 4: Create In-App Banner UI Component (AC: 2)
  - [ ] Create NotificationBannerView.swift in Views/Shared
  - [ ] Design SwiftUI banner: Top of screen, slide-down animation
  - [ ] Display: Sender name (bold), message preview (1 line, truncated)
  - [ ] Style: Semi-transparent background, rounded corners, shadow
  - [ ] Add tap gesture: Navigate to conversation, dismiss banner
  - [ ] Add swipe up gesture: Dismiss banner without navigation
  - [ ] Auto-dismiss after 5 seconds
  - [ ] Support safe area insets (notch-aware)
  - [ ] Add accessibility labels and VoiceOver support
  - [ ] Add animation: Slide down on appear, slide up on dismiss
  - [ ] Test banner on multiple device sizes

- [ ] Task 5: Implement Local Notification Triggers (AC: 3)
  - [ ] In NotificationManager, implement scheduleLocalNotification()
  - [ ] Create UNMutableNotificationContent with title and body
  - [ ] Title: For 1:1 → "[SenderName]", For group → "[SenderName] in [GroupName]"
  - [ ] Body: Message text preview (truncate to 100 characters)
  - [ ] UserInfo: Include conversationId for tap handling
  - [ ] Sound: Use default system sound
  - [ ] Create UNTimeIntervalNotificationTrigger (immediate: 0.1 seconds)
  - [ ] Add notification request to UNUserNotificationCenter
  - [ ] Handle errors (e.g., permissions not granted)
  - [ ] Add debug logging for all notification triggers
  - [ ] Add unit tests for notification payload construction

- [ ] Task 6: Implement Notification Tap Handling & Navigation (AC: 4)
  - [ ] Implement UNUserNotificationCenterDelegate
  - [ ] Set delegate in AppDelegate or MessageAIApp
  - [ ] Implement userNotificationCenter(_:didReceive:withCompletionHandler:)
  - [ ] Extract conversationId from notification userInfo
  - [ ] Navigate to ChatView for that conversationId
  - [ ] Use NavigationPath or state management for deep linking
  - [ ] Handle case where conversation no longer exists (graceful error)
  - [ ] Dismiss in-app banner if showing when notification tapped
  - [ ] Update app state to mark conversation as viewed
  - [ ] Add debug logging for navigation events
  - [ ] Test navigation from background and foreground states

- [ ] Task 7: Implement Smart Suppression Logic (AC: 5)
  - [ ] In NotificationManager, track currentlyViewedConversationId
  - [ ] Update currentlyViewedConversationId when ChatView appears/disappears
  - [ ] In shouldSuppressNotification(): Compare new message conversationId with currently viewed
  - [ ] Return true (suppress) if IDs match, false otherwise
  - [ ] Update ChatView.onAppear to set currentlyViewedConversationId
  - [ ] Update ChatView.onDisappear to clear currentlyViewedConversationId
  - [ ] Ensure suppression works for both in-app banner and local notifications
  - [ ] Add debug logging for suppression decisions
  - [ ] Add unit tests for suppression logic scenarios
  - [ ] Test: User in Chat A, receives message for Chat B → show notification
  - [ ] Test: User in Chat A, receives message for Chat A → suppress notification

- [ ] Task 8: Handle Notification Permission Denial (AC: 2, 3)
  - [ ] Detect when user denies notification permissions
  ...
  - [ ] Cache denial status to avoid repeated prompts
  - [ ] If denied: In-app banner still works (doesn't require permissions)
  - [ ] If denied: Local notifications silently fail (expected behavior)
  - [ ] Log permission denial for debugging
  - [ ] Add UI tests for permission denial flow
  - [ ] Test settings navigation button functionality
...
- [ ] Task 10: Create Unit Tests for NotificationManager (AC: 1-6)
  - [ ] Create NotificationManagerTests.swift for unit tests
  - [ ] Test: Suppression logic (viewing vs not viewing conversation)
  - [ ] Test: Permission request and UserDefaults storage
  - [ ] Test: Notification payload construction (1:1 and group)
  - [ ] Test: Banner auto-dismiss timeout logic
  - [ ] Review existing test files for notification-related impacts
  - [ ] Add mock NotificationManager to test fixtures if needed
  - [ ] Verify all existing tests pass with NotificationManager added
