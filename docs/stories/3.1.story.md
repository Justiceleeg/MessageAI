...
- [ ] Task 1: Update Data Models to Support Group Chats (AC: 1, 2)
  - [ ] Review existing Conversation model - verify `isGroupChat` boolean field exists
  - [ ] Review existing Conversation model - verify `participants` array can hold 3+ userIds
  - [ ] Add optional `groupName` field to Conversation model (String?, for future use)
  - [ ] Update Conversation initializer to properly set `isGroupChat = true` when participants.count > 2
  - [ ] Update ConversationEntity (SwiftData) to include `groupName` field (String?)
  - [ ] Add SwiftData migration if needed for new groupName field
  - [ ] Verify Message and MessageEntity models work for group chats (no changes expected)
  - [ ] Add unit tests for Conversation model with 3+ participants
  - [ ] Add unit tests for isGroupChat flag calculation

- [ ] Task 2: Implement Group Chat Creation UI (AC: 1)
  - [ ] Design UserSearchView to support multi-select mode
  - [ ] Add @State var selectedUsers: [User] to UserSearchView
  - [ ] Update UserSearchView row UI to show checkmarks for selected users
  - [ ] Add "Next" or "Create Group" button that enables when 2+ users selected
  - [ ] Add optional group name input screen (simple TextField, optional for MVP)
  - [ ] Update ConversationListView "New Message" button to navigate to multi-select UserSearchView
  - [ ] Wire up creation flow: UserSearchView → optional name entry → create conversation
  - [ ] Update FirestoreService.createConversation() to handle 3+ participants
  - [ ] Set isGroupChat = true when participants.count > 2 in FirestoreService
  - [ ] Test group chat creation with 3 users, 4 users, and 10 users
  - [ ] Add unit tests for multi-select user search logic

- [ ] Task 3: Update Conversation List UI for Group Chats (AC: 2)
  - [ ] Update ConversationListView row to detect group chats (check conversation.isGroupChat)
  - [ ] For group chats, display participant names instead of single user name
  - [ ] Format participant list: "Alice, Bob, Charlie" or "Alice, Bob, +2 more"
  - [ ] If groupName exists, display groupName instead of participant list
  - [ ] Add group chat icon (SF Symbol: person.3.fill) to distinguish from 1:1 chats
  - [ ] Ensure lastMessageText preview works correctly for group messages
  - [ ] Show sender name prefix for group message previews: "Alice: Hey everyone!"
  - [ ] Test UI with various group sizes (3, 4, 10 participants)
  - [ ] Test UI with and without groupName set
  - [ ] Verify conversation list sorting works correctly for group chats
  - [ ] Add unit tests for participant name formatting logic

- [ ] Task 4: Update Chat View for Group Conversations (AC: 3, 4)
  - [ ] Update ChatView navigation title to show group name or participant list
  - [ ] For group chats without name, show "Alice, Bob, Charlie" in nav title
  - [ ] Truncate long participant lists in nav title: "Alice, Bob, +3 more"
  - [ ] Update MessageBubbleView to show sender name for messages from others
  - [ ] Position sender name above message bubble (small gray text)
  - [ ] Only show sender name for received messages (not current user's messages)
  - [ ] Ensure sender display name is fetched and cached (via FirestoreService)
  - [ ] Update ChatViewModel to track sender names for message display
  - [ ] Add helper method: getSenderDisplayName(userId: String) -> String
  - [ ] Fetch and cache user display names from Firestore users collection
  - [ ] Test group chat UI with messages from 3+ different senders
  - [ ] Verify message bubbles align correctly (sent vs. received)
  - [ ] Add unit tests for sender name display logic

- [ ] Task 5: Ensure Real-time Delivery for Group Chats (AC: 4)
  - [ ] Verify existing Firestore listener in ChatViewModel works for group chats
  - [ ] Verify messages written to Firestore with multiple participants trigger listeners
  - [ ] Test real-time message delivery: send from User A, verify Users B & C receive
  - [ ] Verify Firestore Security Rules allow read access for all participants
  - [ ] Update Firestore rules if needed: participants.includes(request.auth.uid)
  - [ ] Test with 3 simultaneous users sending messages
  - [ ] Verify message ordering is consistent across all participants
  - [ ] Test network latency scenarios (slow connection)
  - [ ] Add integration tests for multi-user real-time sync

- [ ] Task 6: Ensure Offline Persistence for Group Chats (AC: 5)
  - [ ] Verify SwiftData caching works for group conversations
  - [ ] Verify ConversationEntity saves/loads with 3+ participants
  - [ ] Verify MessageEntity caching works for group messages
  - [ ] Test offline queue: send group message while offline
  - [ ] Verify queued group messages are sent when network reconnects
  - [ ] Verify OfflineMessageQueue handles group conversation IDs correctly
  - [ ] Test cache-first loading for group chat message history
  - [ ] Verify optimistic UI works for group messages (instant display)
  - [ ] Test app restart: verify group conversations and messages persist
  - [ ] Add unit tests for SwiftData group chat persistence
  - [ ] Add integration tests for offline group message queue

- [ ] Task 7: Update FirestoreService for Group Operations (AC: 1, 4)
  - [ ] Add or update createGroupConversation(participants: [String], groupName: String?) method
  - [ ] Ensure conversation document created with all participant userIds in array
  - [ ] Set isGroupChat = true in Firestore document
  - [ ] Add/update fetchUserProfile(userId: String) to get display names
  - [ ] Cache user profiles in memory to avoid repeated Firestore reads
  - [ ] Ensure sendMessage() works identically for 1:1 and group conversations
  - [ ] Verify Firestore indexes support array-contains queries on participants
  - [ ] Add error handling for invalid participant lists (< 1 user)
  - [ ] Add unit tests for createGroupConversation method
  - [ ] Add unit tests for user profile fetching and caching

- [ ] Task 8: Update ConversationListViewModel for Groups (AC: 2)
  - [ ] Verify existing Firestore query fetches group conversations
  - [ ] Query: conversations where participants array-contains currentUserId
  - [ ] Verify group conversations appear in conversation list
  - [ ] Test conversation list with mix of 1:1 and group chats
  - [ ] Ensure real-time updates work when new group message arrives
  - [ ] Verify conversation list sorting by lastMessageTimestamp works for groups
  - [ ] Add unit tests for mixed conversation list (1:1 + groups)

- [ ] Task 9: Handle Edge Cases and Error Scenarios
  - [ ] Validate minimum 3 participants for group chat creation
  - [ ] Show error if user tries to create group with < 3 total users (including self)
  - [ ] Handle case where participant userId is invalid or doesn't exist
  - [ ] Handle participant leaving group (post-MVP, for now ensure messages still work)
  - [ ] Test with maximum participants (e.g., 50 users)
  - [ ] Verify UI doesn't break with very long participant lists
  - [ ] Test conversation creation failure scenarios
  - [ ] Add user-friendly error messages for all failure cases
  - [ ] Add unit tests for validation logic
  - [ ] Add unit tests for error handling paths

- [ ] Task 10: Integration Testing for Group Chat Workflow (AC: 1, 2, 3, 4, 5)
  - [ ] Test complete workflow: Create group → Send message → All receive
  - [ ] Test with 3 live user accounts on Firestore (or simulator)
  - [ ] Test User A creates group with B and C
  - [ ] Test User A sends message, verify B and C receive in real-time
  - [ ] Test User B sends message, verify A and C receive
  - [ ] Test mixed 1:1 and group conversations in same list
  - [ ] Test offline: User A sends group message offline, reconnects, verify delivery
  - [ ] Test app restart: verify group conversation persists
  - [ ] Test cache-first loading for group chats
