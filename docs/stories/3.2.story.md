...
- [ ] Task 1: Extend Message Data Model for Read Receipts (AC: 1, 4)
  - [ ] Review existing Message model - verify status field exists with "sending", "sent" values
  - [ ] Add "delivered" and "read" to status enum values
  - [ ] Update Message model documentation to include all status values
  - [ ] Update MessageEntity (SwiftData) to support new status values
  - [ ] Add readBy field to Message model (array of userIds who have read the message)
  - [ ] Add readBy field to MessageEntity (SwiftData) as [String]
  - [ ] Update Message initializer to include readBy parameter (default empty array)
  - [ ] Add SwiftData migration if needed for readBy field
  - [ ] Add unit tests for Message model with readBy field
  - [ ] Add unit tests for all status values

- [ ] Task 2: Update Firestore Schema for Read Receipts (AC: 4)
  - [ ] Update messages sub-collection to include readBy field (array of userIds)
  - [ ] Verify status field in Firestore supports "delivered" and "read" values
  - [ ] Add Firestore index on status field for efficient querying (if needed)
  - [ ] Document Firestore schema changes in architecture docs
  - [ ] Test writing messages with readBy array to Firestore
  - [ ] Test querying messages with readBy array from Firestore

- [ ] Task 3: Implement Message Delivery Status Updates (AC: 1, 2, 4)
  - [ ] In ChatViewModel, detect when user opens ChatView (message delivered)
  - [ ] On ChatView.onAppear, mark all undelivered messages as "delivered"
  - [ ] Create FirestoreService.markMessageAsDelivered(messageId, userId) method
  - [ ] Update message status from "sent" to "delivered" in Firestore
  - [ ] For 1:1 chats: Mark as delivered when recipient's ChatView appears
  - [ ] For group chats: Mark as delivered individually for each recipient
  - [ ] Only mark messages from other users (not own messages) as delivered
  - [ ] Use Firestore batch operations for efficiency when marking multiple messages
...
- [ ] Task 4: Implement Message Read Status Updates (AC: 1, 2, 4)
  - [ ] Detect when user views message in ChatView (message read)
  - [ ] Define "read" as: Message is visible on screen for > 1 second (or immediate if scrolled into view)
  - [ ] Create FirestoreService.markMessageAsRead(messageId, userId) method
  - [ ] Update message status from "delivered" to "read" in Firestore
  - [ ] Add userId to readBy array in Firestore message document
  - [ ] For 1:1 chats: Mark as read when message is visible in recipient's ChatView
  - [ ] For group chats: Add recipient's userId to readBy array
  - [ ] Only mark messages from other users (not own messages) as read
  - [ ] Implement visibility detection using ScrollView position or GeometryReader
  - [ ] Use Firestore batch operations for efficiency when marking multiple messages
  - [ ] Throttle read status updates to avoid excessive Firestore writes
  - [ ] Add error handling for read status update failures
  - [ ] Add unit tests for markMessageAsRead method
  - [ ] Add integration tests for read status updates

- [ ] Task 5: Update Message Status Logic for Read Receipts (AC: 1, 3)
  - [ ] Update ChatViewModel to compute message display status
  - [ ] For 1:1 chats: Status is "read" if readBy contains recipient's userId
  - [ ] For group chats: Status is "read" if readBy contains ALL other participant userIds
  - [ ] Create helper method: computeMessageStatus(message, conversation, currentUserId) -> String
  - [ ] Return "sending" if status is "sending" (optimistic UI)
  - [ ] Return "sent" if status is "sent" (confirmed by Firestore)
  - [ ] Return "delivered" if status is "delivered" (recipient received)
  - [ ] Return "read" if all recipients have read (based on readBy array)
  - [ ] Cache conversation participant list to avoid repeated lookups
  - [ ] Update status computation when readBy array changes (real-time)
  - [ ] Add unit tests for status computation logic
  - [ ] Add tests for 1:1 vs group chat status logic

- [ ] Task 6: Update Message Status UI Indicators (AC: 1, 2)
  - [ ] Update MessageBubbleView to display new status indicators
  - [ ] Extend existing status indicator from Story 2.3
  - [ ] "sending" status: Gray spinner (existing)
  - [ ] "sent" status: Single gray checkmark ✓ (update from blue to gray)
  - [ ] "delivered" status: Double gray checkmark ✓✓
  - [ ] "read" status: Double blue checkmark ✓✓ (filled)
  - [ ] Position status indicator to the right of timestamp (existing pattern)
  - [ ] Use SF Symbols: checkmark (single), checkmark.circle.fill (double)
  - [ ] Follow iOS Messages/WhatsApp status indicator patterns
  - [ ] Make indicators subtle and non-intrusive
  - [ ] Add smooth transitions when status changes (fade animation)
  - [ ] Add accessibility labels for all status states
  - [ ] Test status indicator rendering for all states
  - [ ] Add unit tests for status indicator logic

- [ ] Task 7: Implement Real-time Status Updates (AC: 2)
  - [ ] Verify existing Firestore listener in ChatViewModel captures status changes
  - [ ] Ensure listener receives updates when message status field changes
  - [ ] Ensure listener receives updates when readBy array is updated
  - [ ] Update local @Published messages array when status changes
  - [ ] Update SwiftData cache when status changes
  - [ ] Test real-time status updates: Send from User A, mark read from User B, verify A sees update
  - [ ] Test with multiple recipients in group chat
  - [ ] Verify UI updates immediately when status changes
  - [ ] Add integration tests for real-time status synchronization

- [ ] Task 8: Handle Group Chat Read Status Logic (AC: 3)
  - [ ] For group chats, track readBy array with multiple userIds
  - [ ] Compute "read" status only when ALL other participants have read
  - [ ] Example: 3-person group (A, B, C), A sends message, status is "read" when readBy contains [B, C]
  - [ ] Display partial read status in UI (optional for MVP): "Read by 2 of 3"
  - [ ] For MVP: Simple "read" indicator when all have read, otherwise show "delivered"
  - [ ] Update computeMessageStatus() to check all participants
  - [ ] Get participant list from conversation.participants
  - [ ] Exclude current user from "all recipients" check
  - [ ] Test group read status with 3, 4, and 5 participants
  - [ ] Add unit tests for group read status computation
  - [ ] Add integration tests with multiple users marking messages as read

- [ ] Task 9: Optimize Firestore Writes for Read Receipts (AC: 4)
  - [ ] Batch multiple read status updates together (e.g., mark 10 messages as read in one batch)
  - [ ] Throttle read status updates: Wait 1-2 seconds before writing to Firestore
  - [ ] Avoid updating status if already marked as read
  - [ ] Use Firestore FieldValue.arrayUnion() to add userId to readBy array (atomic operation)
  - [ ] Check if userId already in readBy before updating
  - [ ] Implement debouncing for status updates when scrolling through messages
  - [ ] Queue status updates and flush periodically or on view disappear
  - [ ] Add error handling and retry logic for failed status updates
  - [ ] Test batching with large message lists (50+ messages)
  - [ ] Add unit tests for batching and throttling logic

- [ ] Task 10: Update FirestoreService with Read Receipt Methods (AC: 4)
  - [ ] Add markMessageAsDelivered(conversationId, messageId, userId) method
  - [ ] Implementation: Update message document, set status to "delivered"
  - [ ] Add markMessageAsRead(conversationId, messageId, userId) method
  - [ ] Implementation: Update message document, set status to "read", add userId to readBy array
  - [ ] Use Firestore transactions or batch writes for atomicity
  - [ ] Add error handling for network failures
  - [ ] Return Result<Void, Error> for status
...
- [ ] Task 11: Handle Edge Cases and Error Scenarios (AC: 2, 4)
  - [ ] Handle offline scenarios: Queue status updates, send when reconnected
  - [ ] Use existing NetworkMonitor and offline queue from Story 2.3
  - [ ] Handle case where message is deleted before status updated
  - [ ] Handle case where user leaves group chat (don't wait for their read status)
  - [ ] Handle rapid status changes (sent → delivered → read in quick succession)
  - [ ] Ensure status only progresses forward (never "read" → "sent")
  - [ ] Handle Firestore permission errors gracefully
  - [ ] Show error message if status update fails repeatedly
  - [ ] Add validation: Only message sender sees read receipts
  - [ ] Received messages don't show status indicators
  - [ ] Add unit tests for edge cases
  - [ ] Add integration tests for error scenarios

- [ ] Task 12: Privacy Considerations (Optional for MVP)
  - [ ] (MVP: Always send read receipts, no opt-out)
  - [ ] (Post-MVP: Add user setting to disable sending read receipts)
  - [ ] (Post-MVP: Respect recipient's read receipt settings)
  - [ ] Document privacy approach in Dev Notes
  - [ ] For MVP, all read receipts are always sent

- [ ] Task 13: Update Message Sending Flow with Initial Status (AC: 1)
  - [ ] Verify ChatViewModel.sendMessage() sets initial status to "sending"
  - [ ] After Firestore confirmation, update status to "sent"
  - [ ] Ensure new messages initialize with empty readBy array
  - [ ] Test that sent messages show "sent" status after confirmation
  - [ ] Test that status progresses from "sending" → "sent" → "delivered" → "read"
