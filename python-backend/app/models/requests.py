...
class DecisionCreateRequest(BaseModel):
    """Request for storing decision vector in Pinecone (Story 5.2)"""
    decisionId: str = Field(..., description="Decision ID (generated by iOS client)")
    text: str = Field(..., description="Decision text (complete statement)")
    userId: str = Field(..., description="User who confirmed the decision")
    conversationId: str = Field(..., description="Conversation where decision was made")
    messageId: str = Field(..., description="Message that triggered this decision")
...
class DecisionSearchRequest(BaseModel):
    """Request for searching decisions (Story 5.2)"""
    user_id: str = Field(..., description="User ID for filtering results")
    query: str = Field(..., description="Search query")
    conversation_id: Optional[str] = Field(None, description="Optional: filter by conversation")
