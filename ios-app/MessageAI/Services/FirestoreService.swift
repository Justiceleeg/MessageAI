...
class FirestoreService: ObservableObject {
    ...
    /// - Throws: FirestoreError if the operation fails
    func updateUserProfile(userId: String, displayName: String? = nil, email: String? = nil)
        async throws
    {
        ...
        do {
            ...
            let conversation = snapshot.documents
                ...
                .first { conv in
                    ...
                    conv.participants.contains(userId1) && conv.participants.contains(userId2)
                        && conv.participants.count == 2 && !conv.isGroupChat
                }
        ...
        return AsyncThrowingStream { continuation in
            let listener = db.collection("conversations")
                ...
                .addSnapshotListener { [weak self] snapshot, error in
                    ...
                    if let error = error {
                        self.logger.error(
                            "Failed to listen to conversations: \(error.localizedDescription)")
                        continuation.finish(
                            throwing: FirestoreError.readFailed(error.localizedDescription))
                        return
                    }
                    ...
                    let conversations = documents.compactMap { doc -> Conversation? in
                        guard let conversation = Conversation(document: doc) else {
                            self.logger.warning(
                                "Failed to parse conversation document: \(doc.documentID)")
                            return nil
                        }
                        return conversation
                    }
                    ...
                    self.logger.info(
                        "Fetched \(conversations.count) conversations for userId: \(userId)")
        ...
        return AsyncThrowingStream { continuation in
            let listener = db.collection("conversations")
                ...
                .addSnapshotListener { [weak self] snapshot, error in
                    ...
                    if let error = error {
                        self.logger.error(
                            "Failed to listen to messages: \(error.localizedDescription)")
                        continuation.finish(
                            throwing: FirestoreError.readFailed(error.localizedDescription))
                        return
                    }
                    ...
                    guard let documents = snapshot?.documents else {
                        self.logger.warning(
                            "No message documents found for conversationId: \(conversationId)")
                        continuation.yield([])
                        return
                    }
                    ...
                    let messages = documents.compactMap { doc -> Message? in
                        guard let messageId = doc.data()["messageId"] as? String,
                            ...
                            self.logger.warning(
                                "Failed to parse message document: \(doc.documentID)")
                            return nil
                        }
    ...
    open func sendMessage(
        conversationId: String, senderId: String, text: String, messageId: String? = nil
    ...
    /// - Throws: FirestoreError if the operation fails
    open func createConversationWithMessage(
        participants: [String], senderId: String, text: String, messageId: String? = nil
    ) async throws -> (conversationId: String, message: Message) {
        ...
        do {
            ...
            logger.error(
                "Failed to create conversation with message: \(error.localizedDescription)")
            throw FirestoreError.writeFailed(error.localizedDescription)
        }
    }
    ...
    func getConversation(conversationId: String) async throws -> Conversation {
        ...
        do {
            let document = try await db.collection("conversations").document(conversationId)
                .getDocument()
    ...
    /// - Throws: FirestoreError if the operation fails
    func markMessageAsDelivered(conversationId: String, messageId: String, userId: String)
        async throws
    {
    ...
    /// - Throws: FirestoreError if the operation fails
    func batchMarkMessagesAsRead(conversationId: String, messageIds: [String], userId: String)
        async throws
    {
    ...
    func fetchConversation(conversationId: String) async throws -> Conversation {
        // Delegate to getConversation to avoid code duplication
        return try await getConversation(conversationId: conversationId)
    }
}
